...
# The published swarmcli-cd-oss artefact carries nothing private (D21).
#
# Two artefacts ship from one release: `swarmcli-cd`, built from the private
# wrapper and carrying the licensed code inert, and `swarmcli-cd-oss`, built
# from this tree alone. The second one only means anything if it is verifiably
# the second one — "the released binary is not open source" has a rebuttal only
# while the rebuttal is checkable, and a build published under the wrong name is
# not a thing anyone would notice by reading it.
#
...
# four published binaries and not a locally built stand-in — and by ci.yml on
# every change, so it is not a gate that only ever runs on a tag.
...
main=github.com/Eldara-Tech/swarmcli-cd

# The private modules. Matched as whole module paths or as a parent of one, so
# that `…/swarmcli-cd` is not read as a prefix of `…/swarmcli-cd-be` in either
# direction, and `…/swarmcli` — which is CE, Apache-2.0, and legitimately linked
...
if [ "$got_main" != "$main" ]; then
  ...
  echo "The swarmcli-cd-oss artefact is the public tree's own build; see docs/editions.md."
  exit 1
fi
